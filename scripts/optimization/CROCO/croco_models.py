...
import time
from scripts.deprecated.robots import RobotUnicycleFirstOrder, RobotCarFirstOrderWithTrailers, \
...
class ActionModelAD(crocoddyl.ActionModelAbstract):
    ...
    def __init__(self, robot, features, use_jit=False):
        ...
        self.step_fn = self.robot.step_CROCO
...
class AM_quadrotor(ActionModelAD, crocoddyl.ActionModelAbstract):
    def __init__(self, robot, features, nr, free_time=False, use_jit=False):
        #print("define action model")
        nu = 5 if free_time else 4
        crocoddyl.ActionModelAbstract.__init__(self, crocoddyl.StateVector(13), nu, nr)
        self.unone = np.zeros(self.nu)
        ActionModelAD.__init__(self, robot, features, use_jit=use_jit)
        #ActionModelAD.__init__(self, Quadrotor(free_time=free_time, semi_implicit=False, normalize=False), features, use_jit=use_jit)
...
class OCP_abstract():
    ...
    def create_problem(self, **kwargs):
        ...
        for i in range(self.T):
            ...
            model = self.actionModel(self.robot,
        ...
        self.terminal_model = self.actionModel(self.robot,
